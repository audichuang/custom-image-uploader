...
module.exports = {
  ...
  config: {
    'defaultImageHost': {
      ...
      'type': 'string',
      'default': 'imgur',
      'enum': ['imgur', 'cloudinary', 'sm_ms'],
      'enumNames': ['Imgur', 'Cloudinary', 'SM.MS'],
    ...
    'cloudinaryUploadPreset': {
      'type': 'string',
      'title': 'Cloudinary Upload Preset',
    ...
    'smmsApiKey': {
      'type': 'string',
      'title': 'SM.MS API Key',
      'description': 'Your SM.MS API Key (get from https://smms.app/home/apitoken)',
      'default': ''
    },
    'showNotifications': {
      'type': 'boolean',
  ...
  insertImage(cm, e) {
    ...
    // 獲取當前配置的圖床
    const currentHost = inkdrop.config.get('image-uploader-plugin.defaultImageHost');
  ...
  uploadToImageHost(imageArrayBuffer, hostName, callback) {
    ...
    // 從單個配置項目獲取設定
    let clientId, cloudName, uploadPreset, apiKey;
    
    if (hostName === 'imgur') {
      clientId = inkdrop.config.get('image-uploader-plugin.imgurClientId');
    } else if (hostName === 'cloudinary') {
      cloudName = inkdrop.config.get('image-uploader-plugin.cloudinaryCloudName');
      uploadPreset = inkdrop.config.get('image-uploader-plugin.cloudinaryUploadPreset');
    } else if (hostName === 'sm_ms') {
      apiKey = inkdrop.config.get('image-uploader-plugin.smmsApiKey');
    }
    
    // 構建端點 URL
    let endpoint = config.endpoint;
    if (config.requiresCloudName && cloudName) {
      endpoint = endpoint.replace('{cloudName}', cloudName);
    }
    ...
    if (config.requiresAuth) {
      if (config.authType === 'clientId' && clientId) {
        xhr.setRequestHeader('Authorization', `Client-ID ${clientId}`);
      } else if (config.authType === 'bearerToken' && apiKey) {
  ...
  isImageHostConfigured(hostName) {
    ...
    if (config.requiresAuth) {
      if (config.authType === 'clientId') {
        const clientId = inkdrop.config.get('image-uploader-plugin.imgurClientId');
        if (!clientId) return false;
      } else if (config.authType === 'bearerToken') {
        const apiKey = inkdrop.config.get('image-uploader-plugin.smmsApiKey');
        if (!apiKey) return false;
      }
    }
    
    if (config.requiresCloudName) {
      const cloudName = inkdrop.config.get('image-uploader-plugin.cloudinaryCloudName');
      if (!cloudName) return false;
    }
    
    if (config.requiresUploadPreset) {
      const uploadPreset = inkdrop.config.get('image-uploader-plugin.cloudinaryUploadPreset');
  ...
  setImageHost(hostName) {
    ...
    inkdrop.config.set('image-uploader-plugin.defaultImageHost', hostName);
