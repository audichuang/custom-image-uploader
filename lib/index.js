...
module.exports = {
  ...
  insertImage(cm, e) {
    ...
    // 獲取當前配置的圖床
    const currentHost = inkdrop.config.get('custom-image-uploader.defaultImageHost');
  ...
  uploadToImageHost(imageArrayBuffer, hostName, callback) {
    ...
    const settings = inkdrop.config.get(`custom-image-uploader.${hostName}`);
  ...
  handleUploadFailure(cm, cursorPosition, originalUrl, error = null) {
    ...
    const retainOriginal = inkdrop.config.get('custom-image-uploader.retainOriginalPath');
    let replacement;
  ...
  setImageHost(hostName) {
    ...
    inkdrop.config.set('custom-image-uploader.defaultImageHost', hostName);
    this.showNotification(`Switched to image host: ${IMAGE_HOST_CONFIGS[hostName].name}`);
  },
  
  showNotification(message, type = 'info') {
    if (!inkdrop.config.get('custom-image-uploader.showNotifications')) return;
